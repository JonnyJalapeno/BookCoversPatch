...
namespace BookCoversPatch
{
    public class Program
    {
        ...
        public static void RunPatch(IPatcherState<ISkyrimMod, ISkyrimModGetter> state)
        {
            ...
            foreach(var book in state.LoadOrder.PriorityOrder.WinningOverrides<IBookGetter>().Where(x => books.Contains(x.FormKey)))
            {
                //var winningOverride = winningOverrides.Where(x => x.FormKey == book.FormKey).Last();
                var patchBook = state.PatchMod.Books.GetOrAddAsOverride(book);
